# class methods, instance methods, stsric methods

# instance methods - обычные методы, которые принимают первым аргументом self (ссылка на объект). Нужны они чтобы внутри метода мы могли работать с атрибутами объекта, получать их или изменять
...
# obj = Test()
# obj1 = Test()
# Test.instance_method(5) # если вызвать у объекта, то self передается автоматически
# Test.instance_method(obj, 5) # если вызвать у класса, то в self нужно передать вручную
# obj1.instance_method(5)

# class methods - методы, которые принимают первым аргументом cls(ссыылка на класс). Нужны они для создания или изменения атрибутов класса и для манипуляций с классом внутри метода. Создается при помощи декоратора @classmethod
...
# a = datetime.now()
# print(a.strftime('%H:%M:%S'), type(a.strftime))

#staticmethod - это те методы в классе, которые не принимают в качестве аргументов ни класс ни объект, так называемые которые не изменяют состояние
