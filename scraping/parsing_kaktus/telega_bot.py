...
def start_parse(message):
    ...
    bot_message = bot.send_message(message.chat.id, 'Choice number of articles for detail info(1-20): ')
...
def show_info(message, number, data):
    if message.text.lower() == 'photo':
        ...
        bot_message = bot.send_message(message.chat.id, 'Choice number of articles for detail info(1-20):')
        bot.register_next_step_handler(bot_message, check_number, message.text, data)
