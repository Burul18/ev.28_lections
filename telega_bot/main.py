...
token = '6058190058:AAHWcPwd2ImcnA4qpIeB551SUOLlUcQwHzg'
...
def check_answer(message):
    if message.text == 'Играть!':
        bot.send_message(message.chat.id, 'Ok, тогда лови правила игры:\nНужно угадать число, которое я загадаю в диапазоне от 1 до 10 включительно! У тебя будет 3 попытки! Если не угадаешь я тебя повешу! :)')
...
def check_number(message, attempts, number):
    if message.text == str(number):
        ...
        bot.send_message(message.chat.id, 'You\'re lost again and again!\nBut you\'re still loking at your dream!\nIt\'s not over untill you win')
    else:
        bot.send_message(message.chat.id, 'Нет ты не угудал число, попробуй еще раз!')
